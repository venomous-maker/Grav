...
| `math.grav`      | **`Math`** class — integer `abs`, `min`, `max`, `sign`, `clamp`, `pow`, `isqrt`, `gcd`, `lcm`, `factorial`. |
| `mathf.grav`     | **`MathF`** class — floating-point `sqrt`, `cbrt`, `pow`, `abs`, `sin`, `cos`, `tan`, `atan2`, `floor`, `ceil`, `round`, `log`, `log10`, `exp`, `min`, `max`, `pi()`, `e()` (over `<math.h>`). |
